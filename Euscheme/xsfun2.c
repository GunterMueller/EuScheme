...
/* external routines */
extern void xlprin1(),xlprinc(), check_if_disabled();
...
#else
static void do_maploop();
static void do_forloop();
static void do_withfile();
static void do_load();
static void do_loadloop();
static LVAL setit();
static LVAL openfile();
static LVAL strcompare();
static LVAL chrcompare();
extern LVAL ospeekchar();
#endif
...
LVAL xcurinput()
{
   static char *cfn_name = "current-input-port";
   xllastarg();
    return (curinput());
}
...
LVAL xcuroutput()
{
    static char *cfn_name = "current-output-port";
    xllastarg();
    return (curoutput());
}
...
LVAL default_handler()
{
  ...
  for (i = 1; sds; i++, sds = cdr(sds)) {
    if (getivar(condition, i) == s_unbound) continue;
