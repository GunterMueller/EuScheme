...
extern void
    ...
    xload(),xloadnoisily(),xload1(), xinitloop1(), xinitialize_object();
extern LVAL
    xcons(),xcar(),xcdr(),
    ...
    xboundp(),xget(),xput(),xgetsyntax(),xputsyntax(),
    ...
    xpairp(),xsymbolp(),xkeywordp(),xintegerp(),
    xrealp(),xcharp(),xstringp(),xvectorp(),
    ...
    xstrlen(),xstrnullp(),xstrappend(),xstrref(),xstrset(),xsubstring(),
    xstrlist(),xliststring(),
    xstrlss(),xstrleq(),xstreql(),xstrgeq(),xstrgtr(),
    xstrilss(),xstrileq(),xstrieql(),xstrigeq(),xstrigtr(),xmakestring(),
    xcharint(),xintchar(),xstringnum(), xnumstring(),
    ...
    xgetfposition(),xsetfposition(),xunlink(),xcurinput(),xcuroutput(),
    ...
    module_symbols(), module_exports(), symbol_module(), xfind_module(),
    current_mod(), mod_list(), unintern(), xkeyword_array(), xmake_constant(),
    xallocate(), xfind_key(), xtelos_error(),
    xinitialize_class(), xclassof(), xdescribe(),
    xclass_name(), xclass_superclasses(), xclass_subclasses(), xclass_slots(),
    xclass_keywords(), xset_class_keywords(), xclass_instsize(),
    xclass_abstractp(), xclass_cpl(), xclassp(), xsubclassp(),
    xgf_name(), xgf_args(), xgf_setargs(), xgf_optargs(),
    xgf_methods(), xgf_cache1(), xgf_cache2(),
    xmethod_gf(), xmethod_fun(), xmethod_domain(), xmake_and_add_method(),
    xmake_generic(), xmake_method(), xadd_method(),
    xslot_name(), xslot_keyword(), xslot_default(), xset_slot_default(),
    xslot_requiredp(), xset_slot_requiredp(),
    xfind_slot_index(), xmake_table(), xtable_ref(), xtable_set(),
    xtable_comparator(), xtable_delete(), xtable_length(),
    xtable_keys(), xtable_values(),
    xbacktrace(), xset_module(),
    xreintern(), xreintern_syntax(), xmodule_directives(),
    xtable_fill(), xtable_setfill(), xtable_clear(), xmacro_error(),
    xsetivar(), xgetivar(), xtmpfile(), xgetenv(), xputenv(), xtime(),
    xsprintf(), xframe_up(), xframe_down(), xframe_env(), xframe_fun();
#ifndef NO_CHECK_REF
extern LVAL xcheck_ref();
#endif
#ifdef SOCK
extern LVAL socket_socket(), socket_connect(),
    socket_bind(), socket_listen(), socket_accept(), socket_block(),
    socket_nonblock(), socket_reuse(), socket_noreuse(), socket_close(),
    socket_shutdown(), socket_peeraddr(), socket_peerport(),
    socket_sockaddr(), socket_sockport(), socket_host_to_ip(),
    socket_ip_to_host(), socket_fd_zero_read(), socket_fd_set_read(),
    socket_fd_isset_read(), socket_select_read(), socket_fd_zero_write(),
    socket_fd_set_write(), socket_fd_isset_write(), socket_select_write(),
    socket_convert_to_port(), port_fd(), port_unbuffered(),
    port_block_buffered(), port_line_buffered(), port_xdr_send_int(),
    port_xdr_recv_int(),
    port_xdr_send_float(), port_xdr_recv_float(), port_xdr_send_string(),
    port_xdr_recv_string();
#endif
...
{       "slot-required-p",                      xslot_requiredp	},
{       "set-slot-required-p!",                 xset_slot_requiredp },
  ...
  char *name, *fn;
{
    ...
    if (cond != s_unbound) {
      class = name[0] == '<' ?
              getvalue(xlenter_module(name, root_module)) :
              cvstring(name);
