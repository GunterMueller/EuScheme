...
static int findvmemory();
static void compact();
void gc();
static void mark();
static void markvector();
static void compact_vector();
static void sweep();
static void sweep_segment();
static void badobjtype();
#endif
...
static void set_symbol_name(new, pname)
  LVAL new;
  char *pname;
{
  LVAL sym, name;
  ...
  char *name;
{
    ...
    /* next line to ensure that oblists of different modules differ,
       and are not compiled into equal literals */
  ...
  int reason;
{
    ...
    if (!quiet && s_gcmsgs && getvalue(s_gcmsgs)) fprintf(stderr, "C>");
#if 0
    pstack();
#endif
