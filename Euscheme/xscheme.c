...
extern LVAL s_stdin,s_stdout,s_stderr,s_unbound,s_backtracep;
extern LVAL s_general_error, s_user_intr, s_signal;
extern int trace;
int quiet, no_system;
FILE *filein;
  ...
  int argc; char **argv;
{
	...
	if (argv[src][0] == '-') {
		...
		case 'q':
		    ...
		    break;
		case 'f':	/* in shell script */
		    src++;
		    if (src >= argc) {
		      fprintf(stderr, "euscheme: missing input file\n");
		      exit(5);
		    }
		    filein = osaopen(argv[src], "r");
		    if (filein == NULL) {
		      fprintf(stderr, "euscheme: can't open input '%s'\n",
			      argv[src]);
		      exit(6);
		    }
		    do {	/* skip #! line */
...
static void usage()
{
    fprintf(stderr, "usage: euscheme [-tqnsf] [file] [arg ...]\n");
    exit(1);
}
  ...
  int cc;
{
  ...
  if (cond == s_unbound || xlval == s_unbound) { /* no conditions yet */
    errputstr("Run-Time ");
    xlerror(msg, arg);
  }
  else {
    ...
    if (getsfixnum(getivar(condcl, INSTSIZE)) > 1) setivar(cond, 2, arg);
    oscheck();
...
static void trace_function(fun, env)
  LVAL fun, env;
{
