...
static void merge_slots(), merge_keywords(), do_initloop();
static LVAL redefine_slot();
static int slotequal();
int xlsubclassp();
extern LVAL true, s_unbound, cs_initloop1, xlenv, xlval;
extern LVAL s_direct_slots, s_direct_keywords, s_name, s_default,
            s_requiredp, s_keyword, s_telos_error, s_incompatible_md;
extern LVAL xlmember(), xlenter_keyword();
extern int eq();
...
{
  /* small integers */
  if (!ispointer(obj)) return getelement(class_vector, FIXNUM);

  if (obj == NULL) return getelement(class_vector, NULLTYPE);

  if (objectp(obj)) return getclass(obj);
  ...
  if (portp(obj)) {
	...
	return getelement(class_vector, IOPORT); /* input/output port */
      else
  ...
  LVAL obj;
{
  ...
  if (slotp(obj)) {
    ...
    if (getslotrequiredp(obj) == NIL) xlputstr(fptr, "not ");
...
LVAL find_key(key, inits, deefault)
...
static void check_legal_keywords(inits, keys)
  LVAL inits, keys;
{
  for (; inits; inits = cdr(cdr(inits)))
    if (xlmember(car(inits), keys, eq) == NIL)
      xlcerror("illegal keyword in initialize", car(inits), s_telos_error);
}

static void check_required_keywords(inits, slots)
  LVAL inits, slots;
{
  LVAL key, ints;

  for (; slots; slots = cdr(slots)) {
    if (getslotrequiredp(car(slots)) == NIL) continue;
    key = getslotkey(car(slots));
    for (ints = inits; ints; ints = cdr(cdr(ints))) {
      if (key == car(ints)) break;
    }
    if (ints == NULL)
...
static void do_initloop(index, slots, inits)
  int index;
  LVAL slots, inits;
{
  ...
  if (init == NIL) {
    if (closurep(defaultfn)) {	/* no initarg, a defaultfn */
      check(6);
...
void xinitialize_object()
{
  ...
  if (!objectp(xlval))
    ...
    if ((length(inits) & 1) == 1)
      xlcerror("odd-length init list in initialize", inits, s_telos_error);
...
LVAL xinitialize_class()
{
  ...
  if (getivar(super, ABSTRACTP) == NIL)
    xlcerror("only subclassing of abstract classes", super,
	     s_telos_error);
#endif
#endif
...
static void merge_slots(obj, super, inits)
  LVAL obj, super, inits;
{
  ...
  for (; dirslots; dirslots = cdr(dirslots)) {
	...
	xlcerror("keyword required with no keyword defined",
      ...
      slots = redefine_slot(slotname, defn, slotkey, reqd, slots, slotdesc);
    }
    drop(1);
  }
...
static int slotequal(name, slot)
  LVAL name, slot;
{
#ifndef OLDSYM
...
*/
static LVAL redefine_slot(slotname, defn, slotkey, reqd, slots, slotdesc)
  LVAL slotname, defn, slotkey, reqd, slots, slotdesc;
{
  LVAL old, new;
...
static void merge_keywords(obj, super, inits)
  LVAL obj, super, inits;
{
  LVAL dirkeys, keys;
...
static LVAL init_class(type, name, super, absp)
  int type;
  char *name;
  LVAL super, absp;
{
  ...
  if (type >= 0) setelement(class_vector, type, cl);
...
static int applicablep(md, domain)
  LVAL md, domain;
{
  LVAL md_domain;
...
static LVAL applicable_methods(gf, classes)
 LVAL gf, classes;
{
  ...
  if (app == 0) return NIL;
  if (app == 1) return cons(buf[0], NIL);

#if 0
  { int i;
    xlputstr(curoutput(), "<applic");
    for (i = 0; i < app; i++) {
      xlputstr(curoutput(), " ");
      xlprin1(buf[i], curoutput());
    }
    xlputstr(curoutput(), ">\n");
  }
  qsort((char *)buf, app, sizeof(LVAL), specific);
  { int i;
    xlputstr(curoutput(), "<sorted");
    for (i = 0; i < app; i++) {
      xlputstr(curoutput(), " ");
      xlprin1(buf[i], curoutput());
    }
...
LVAL find_and_cache_methods(gf, arglist)
  LVAL gf, arglist;
{
...
static LVAL add_or_replace_method(md, mdlist)
  LVAL md, mdlist;
{
  LVAL domain, list;
...
static void check_domain_compatibility(domain, gf, optargs)
  LVAL domain, gf, optargs;
{
  LVAL gfdomain;
...
#ifndef NO_CHECK_REF
void telos_bad_ref_error(object, wanted, interp)
LVAL object, wanted;
int interp;
{
